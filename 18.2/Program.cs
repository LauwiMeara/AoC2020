...
namespace _18._2
{
    class Program
    {
        ...
        static void GetExpressionResult(string[] line, int startIndex, int endIndex, string opr)
        {
            for (int i = startIndex; i < endIndex - 2; i++)
            {
                ...
                // If the operator isn't the desired operator, skip the calculation.
                if (line[expressionIndexes[1]] != opr) continue;
